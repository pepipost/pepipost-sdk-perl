package WWW::Pepipost::ApiClient;
...
sub _new_instance
{
    ...
    my (%args) = (
        ...
        'base_url' => 'http://api.pepipost.com/',
        @_
    );
...
sub _cfg {'WWW::Pepipost::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $WWW::Pepipost::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $WWW::Pepipost::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("WWW::Pepipost::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $WWW::Pepipost::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $WWW::Pepipost::Configuration::api_key_prefix->{$key_name};
