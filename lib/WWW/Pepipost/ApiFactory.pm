package WWW::Pepipost::ApiFactory;
...
usesub WWW::Pepipost::Object;

use WWW::Pepipost::ApiClient;

=head1 Name

	WWW::Pepipost::ApiFactory - constructs APIs to retrieve WWW::Pepipost objects

=head1 Synopsis

	package My::Petstore::App;
	
	use WWW::Pepipost::ApiFactory;
	
	my $api_factory = WWW::Pepipost::ApiFactory->new( ... ); # any args for ApiClient constructor
							  
	# later...
	my $pet_api = $api_factory->get_api('Pet');  
	
	# $pet_api isa WWW::Pepipost::PetApi
...
my %_apis = map { $_ =~ /^WWW::Pepipost::(.*)$/; $1 => $_ } 
			grep {$_ =~ /Api$/} 
			usesub 'WWW::Pepipost';
...
sub new {
    my ($class, %p) = (shift, @_);
	$p{api_client} = WWW::Pepipost::ApiClient->instance(%p);			
